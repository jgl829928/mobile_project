var calUtil = {
    ...
    bindEnvent:function(signList){
      ...
      // //绑定上个月事件
      if(calUtil.showYear == minYear && calUtil.showMonth == month){
        ...
        if(calUtil.showYear == today.getFullYear() && calUtil.showMonth == month){
  ...
  //  });
    ...
    drawCal : function(iYear, iMonth ,signList) {
     ...
     htmls.push("<div class='sign_succ_calendar_title flex_dom flex_item_mid'>");
     htmls.push("<div class='calendar_month_span flex_1'></div>");
     htmls.push("</div>");
     ...
     for (w = 1; w < 6; w++) {
      ...
      for (d = 0; d < 7; d++) {
       ...
       }else if(today.getMonth()+1 == calUtil.showMonth && today.getDate() == myMonth[w][d]){
           htmls.push("<div class='td_"+d+" sign'><span>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</span></div>");
       } else {
